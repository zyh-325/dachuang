...
def get_isicseven_data():
    train_df = pd.read_csv("D:\FG-SSL-main\dataset\data\ISIC-2017_Training_Part3_GroundTruth.csv")
    test_df = pd.read_csv("D:\FG-SSL-main\dataset\data\ISIC-2017_Test_v2_Part3_GroundTruth.csv")

    train_images_dir = "D:\FG-SSL-main\dataset\data\ISIC-2017_Training_Data"
    test_images_dir ="D:\FG-SSL-main\dataset\data\ISIC-2017_Test_v2_Data"
...
def barlow_criterion(x, y, lmbd=0.005, device='cpu'):  # 添加 lmbd 参数并给出默认值
    bs = x.size(0)
    emb = x.size(1)
    ...
    loss = (crossCorMat * lmbd - torch.eye(emb, device=device) * lmbd).pow(2)
